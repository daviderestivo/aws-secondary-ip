...
The script requires an IAM role to allow the monitoring machine to re-create an new machine.
...
*Warning*: The aboce IAM policy grants broad EC2 permissions (ec2:*) and is intended for testing purposes only. For production use, create a more restrictive policy with only the minimum required permissions.
