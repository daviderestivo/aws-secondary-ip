...
1. *Monitoring*: Continuously monitors a target EC2 instance by pinging a specified IP address
2. *Health Check*: Verifies instance state (running) and network reachability
3. *Failover*: When failure is detected, launches a new instance in a different AZ
4. *Route Update*: Updates VPC route tables to redirect traffic to the new instance
5. *Cleanup*: Terminates the failed instance and continues monitoring the replacement
...
- *Multi-AZ VPC*: VPC must span multiple availability zones
- *Subnets*: At least one subnet in each AZ where failover is needed
- *Route Tables*: Custom route tables for directing traffic to secondary IPs
...
to re-create an new machine (do not use it for non-productive
environments):
...
1. *Create Custom Route Tables*: One for each subnet that needs failover
2. *Static Routes*: Add routes pointing to your secondary IP destinations
3. *Associate*: Link route tables to appropriate subnets
...
│   └── Instance A (Primary: 10.0.1.100, Secondary: 10.0.0.10)
├── AZ-B Subnet (10.0.2.0/24)
│   └── Instance B (Primary: 10.0.2.100, Secondary: 10.0.0.10)
└── Route Tables
